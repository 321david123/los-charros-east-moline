/* =========================================================
   LOS CHARROS — interactions & motion
...
(function () {
  ...
  /* ---------------------------------------------------------
     1. Smooth scroll (Lenis) + GSAP ScrollTrigger sync
        ONE rAF driver. Never also drive lenis from gsap.ticker.
  --------------------------------------------------------- */
  let lenis = null;
  ...
  /* ---------------------------------------------------------
     ...
     Mon–Thu 11–19 · Fri 11–20 · Sat 11–16 · Sun closed
  ...
  (function openNow() {
    ...
    const open = 11 * 60;
    let close = null, rowIndex = null;
    if (day >= 1 && day <= 4) { close = 19 * 60; rowIndex = day - 1; }     // Mon–Thu → rows 0–3
    else if (day === 5)       { close = 20 * 60; rowIndex = 4; }            // Fri
    else if (day === 6)       { close = 16 * 60; rowIndex = 5; }            // Sat
    if (rowIndex !== null && mins >= open && mins < close) {
      const rows = $$("#hours tr");
  ...
  /* ---------------------------------------------------------
     12. Magnetic primary buttons (subtle)
  --------------------------------------------------------- */
  if (!reduceMotion && window.matchMedia("(hover:hover)").matches) {
    $$(".btn--red, .btn--gold").forEach(btn => {
